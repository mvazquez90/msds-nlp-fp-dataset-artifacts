...
def main():
    ...
    argp.add_argument('--output_entailed_predictions', action="store_true",
                      help="Output eval on entailed examples from evaluation set.")
    ...
    if training_args.do_eval:
        ...
        if args.output_entailed_predictions:
            with open(os.path.join(training_args.output_dir, 'entailed_predictions.jsonl'), encoding='utf-8', mode='w') as f:
                for i, example in enumerate(eval_dataset):
                    if example['label'] == 0:
                        example_with_prediction = dict(example)
                        example_with_prediction['predicted_scores'] = eval_predictions.predictions[i].tolist()
